<template lang="html">
   <div class="pay">
      ...
      <div class="pay-address">
          <div>
            <p class="main-address-per">收货人:<span>X先生</span></p>
            <p class="main-address-tel">123456</p>
          </div>
          <p>收货地址:<span>XX省XX市XX区XX大街XX路XX号</span></p>
      </div>
        ...
        methods:{
          payConfirm:function () {
            if(this.carList){
              //还没有提交订单，数据没清空
               MessageBox
                 .confirm(`确定支付${this.allpay}元`)
                 .then(action=>{
                   //点击成功执行这里的函数
                   this.confirm=false;
                   this.$store.commit('SET_LOADING',true);
                   this.$store.dispatch('resetCarList');   //重置购物车(用unSelectedList代替carList)
